...
bool WXMixGFNode::getRandomBoolean(float rate) {
...
bool WXMixGFNode::getRandomBoolean() {
...
int WXMixGFNode::getRandomNum(int range) {
...
bool WXMixGFNode::init()
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		imgbtn[i]->setTag(i);
		imgbtn[i]->addTouchEventListener(CC_CALLBACK_2(WXMixGFNode::onImageClick, this));
	...
	mixokbtn->addTouchEventListener(CC_CALLBACK_2(WXMixGFNode::onMix, this));
	...
	okbtn->addTouchEventListener(CC_CALLBACK_2(WXMixGFNode::onOK, this));

	cocos2d::ui::ImageView* suggestbtn = (cocos2d::ui::ImageView*)csbroot->getChildByName("heroatrmixhint");
	suggestbtn->addTouchEventListener(CC_CALLBACK_2(WXMixGFNode::onSuggest, this));
...
void WXMixGFNode::onEnterTransitionDidFinish()
{
	Node::onEnterTransitionDidFinish();
}

int WXMixGFNode::getRandomNum(int rangeStart, int rangeEnd) {
...
void WXMixGFNode::onOK(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void WXMixGFNode::onImageClick(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	if (type == ui::Widget::TouchEventType::ENDED)
	{
...
void WXMixGFNode::shake(Node * node, float scaleLarge, float scaleSmall) {
	if (NULL == node) {
		return;
	}
...
void WXMixGFNode::onMix(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	if (type == ui::Widget::TouchEventType::ENDED)
	{
		SoundManager::getInstance()->playSound(SoundManager::SOUND_ID_BUTTON);
		...
		if (masterGFData != NULL && selectsecsize > 0)
		{
			...
			this->getParent()->addChild(WXMixGFDoneLayer::create(isok), 2);
			if (isok == -1)
			{
...
void WXMixGFNode::shake(Node * node) {
	if (NULL == node) {
		return;
	}
...
void WXMixGFNode::updateGFScroll()
{
	...
	for (int i = 0; i < tempsize; i++)
	{
		...
		MenuItemSprite* boxItem = MenuItemSprite::create(
			...
			box,
			box,
			CC_CALLBACK_1(WXMixGFNode::onItem, this));
...
void WXMixGFNode::jump(cocos2d::Node *node, float dt, bool repeat, float intrval) {
	if (nullptr == node) {
		return;
	}
...
void WXMixGFNode::loadMixSuccGF()
{
	std::string mymixgf = GlobalData::getMixGF();
	if (mymixgf.length() > 0)
	{
...
void WXMixGFNode::jellyJump(cocos2d::Node *node, float dt, bool repeat, float intrval, int tag) {
	if (nullptr == node) {
		return;
	}
...
void WXMixGFNode::onItem(Ref* pSender)
{
	SoundManager::getInstance()->playSound(SoundManager::SOUND_ID_BUTTON);
...
void WXMixGFNode::petJump(cocos2d::Node *node, float dt, bool repeat, float intrval, int tag, ActionInterval *ac) {
	if (nullptr == node) {
		return;
	}
...
void WXMixGFNode::jelly(Node *node, bool repeat, float intrval, bool delay, int tag) {
	if (nullptr == node) {
		return;
	}
...
void WXMixGFNode::onSuggest(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	if (type == ui::Widget::TouchEventType::ENDED)
	{
		SoundManager::getInstance()->playSound(SoundManager::SOUND_ID_BUTTON);
		...
		{
			WXMixSuggestLayer* layer = WXMixSuggestLayer::create();
			g_gameLayer->addChild(layer, 4, "mixsuggestlayer");
		}
	}
}

void WXMixGFNode::updateDesc()
{
	std::string mixid = GlobalData::getMixGF();
	if (mixid.length() > 0)
	{
...
void WXMixGFNode::jumpDown(cocos2d::Node *node, float dt) {
	if (nullptr == node) {
		return;
	}
...
void WXMixGFNode::showTalkGuide()
{
