...
#include "GameScene.h"
WXExerciseDoneLayer::WXExerciseDoneLayer()
{
...
void WXExerciseDoneLayer::jump(cocos2d::Node *node, float dt, bool repeat, float intrval) {
	if (nullptr == node) {
		return;
	}
...
bool WXExerciseDoneLayer::init(std::string wgidstr, std::string ngidstr, int hour)
{
	...
	this->scheduleOnce(schedule_selector(WXExerciseDoneLayer::showText), 0.6f);

	return true;
}

void WXExerciseDoneLayer::jellyJump(cocos2d::Node *node, float dt, bool repeat, float intrval, int tag) {
	if (nullptr == node) {
		return;
	}
...
WXExerciseDoneLayer* WXExerciseDoneLayer::create(std::string wgidstr, std::string ngidstr, int hour)
{
	WXExerciseDoneLayer *pRet = new WXExerciseDoneLayer();
...
void WXExerciseDoneLayer::showText(float dt)
{
	...
	this->scheduleOnce(schedule_selector(WXExerciseDoneLayer::removeself), 3.5f);
}

void WXExerciseDoneLayer::petJump(cocos2d::Node *node, float dt, bool repeat, float intrval, int tag, ActionInterval *ac) {
	if (nullptr == node) {
		return;
	}
...
void WXExerciseDoneLayer::removeself(float dt)
{
	this->removeFromParentAndCleanup(true);
}

void WXExerciseDoneLayer::exerciseDone(std::string wgidstr, std::string ngidstr, int hour)
{
	int f_gfexp = 0;
	int f_heroexp = 0;
...
void WXExerciseDoneLayer::jelly(Node *node, bool repeat, float intrval, bool delay, int tag) {
	if (nullptr == node) {
		return;
	}
