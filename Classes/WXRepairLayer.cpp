...
WXRepairLayer* WXRepairLayer::create(int type)
{
	WXRepairLayer *pRet = new WXRepairLayer();
...
bool WXRepairLayer::init(int type)
{
	m_type = type;
	...
	backbtn->addTouchEventListener(CC_CALLBACK_2(WXRepairLayer::onBack, this));
	...
	m_actionBtn->addTouchEventListener(CC_CALLBACK_2(WXRepairLayer::onAction, this));
	...
	repairResImg->addTouchEventListener(CC_CALLBACK_2(WXRepairLayer::onResItem, this));
	...
	for (int i = 0; i < 3; i++)
	{
		...
		needResImg[i]->addTouchEventListener(CC_CALLBACK_2(WXRepairLayer::onResItem, this));
...
void WXRepairLayer::onEnterTransitionDidFinish()
{
...
void WXRepairLayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void WXRepairLayer::onAction(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	CommonFuncs::BtnAction(pSender, type);
	if (type == ui::Widget::TouchEventType::ENDED)
	{
...
void WXRepairLayer::onRepairItem(cocos2d::Ref* pSender)
{
	Node* node = (Node*)pSender;
...
void WXRepairLayer::onResItem(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	if (type == ui::Widget::TouchEventType::ENDED)
	{
		Node* node = (Node*)pSender;
...
void WXRepairLayer::showAnim()
{
	...
	repairbox->runAction(Sequence::create(EaseSineIn::create(RotateTo::create(1.5f, 720 * 2)), CallFuncN::create(CC_CALLBACK_1(WXRepairLayer::finishAnim, this)), NULL));
}

int WXRepairLayer::getMyPackageResCount(std::string strid)
{
	int count = 0;
...
void WXRepairLayer::finishAnim(Ref* pSender)
{
	float delay = 0.05f;
	...
	if (m_type == 2)
	{
		...
		if (r < succrnd)
		{
			float dt = 0.5f;
			data->slv += 1;
			this->scheduleOnce(schedule_selector(WXRepairLayer::delayShowOkLayer), dt);
			delay += dt;
	...
	updataUI();
	this->scheduleOnce(schedule_selector(WXRepairLayer::delayCanClick), delay);
}

void WXRepairLayer::updataUI()
{
	...
	for (unsigned int i = 0; i < vec_RepairData.size(); i++)
	{
		...
		MenuItemSprite* boxItem = MenuItemSprite::create(
			...
			box,
			box,
			CC_CALLBACK_1(WXRepairLayer::onRepairItem, this));
...
void WXRepairLayer::delayShowOkLayer(float dt)
{
	WXRepairOKLayer* layer = WXRepairOKLayer::create((PackageData*)repairResImg->getUserData(), m_type);
	this->addChild(layer);
...
void WXRepairLayer::delayCanClick(float dt)
{
	g_gameLayer->removeChildByName("notouchlayer");
}

void WXRepairLayer::dofail()
{
