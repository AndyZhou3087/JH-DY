...
#include "GameScene.h"
bool WXAchiveDoneAnimLayer::init()
{
...
void WXAchiveDoneAnimLayer::initA()
{
...
WXAchiveDoneAnimLayer* WXAchiveDoneAnimLayer::create()
{
	WXAchiveDoneAnimLayer *pRet = new WXAchiveDoneAnimLayer();
	if (pRet && pRet->init())
	{
		pRet->autorelease();
	}
...
void WXAchiveDoneAnimLayer::showAchiveDoneAnim(float dt)
{
	...
	this->schedule(schedule_selector(WXAchiveDoneAnimLayer::showAchiveDoneAnim), delaytime, GlobalData::vec_showAchiveNames.size(), 0);
}

void WXAchiveDoneAnimLayer::initBg()
{
	const Size size = Director::getInstance()->getVisibleSize();
...
void WXAchiveDoneAnimLayer::removSelf()
{
