...
Scene* WXActivitScene::createScene(std::string imagepath, std::string content)
{
	...
	auto layer = WXActivitScene::create(imagepath, content);
...
bool WXActivitScene::init(std::string imagepath, std::string content)
{
	...
	else if (imagepath.compare("images/thieves.jpg") == 0)
	{
		...
		{
			this->scheduleOnce(schedule_selector(WXActivitScene::checkstoleData), 0.1f);
			distime = 3.5f;
	...
	else if (imagepath.compare("images/findtreasure.jpg") == 0)
	{
		...
		else
		{
			...
			{
				this->scheduleOnce(schedule_selector(WXActivitScene::getRndRes), 0.2f);
	...
	this->scheduleOnce(schedule_selector(WXActivitScene::popself), distime);
	return true;
}

void WXActivitScene::saveTime() {
	
}

void WXActivitScene::on1sTimer() {
	if (m_countdown-- <= 0) {
		if (500 < 20) {
...
void WXActivitScene::checkstoleData(float dt)
{
...
void WXActivitScene::getRndRes(float dt)
{
	...
	this->scheduleOnce(schedule_selector(WXActivitScene::popself), 4.0f);
}


void WXActivitScene::initBg()
{
	const Size size = Director::getInstance()->getVisibleSize();
...
WXActivitScene* WXActivitScene::create(std::string imagepath, std::string content)
{
	WXActivitScene *pRet = new WXActivitScene();
...
void WXActivitScene::popself(float dt)
{
	Director::getInstance()->popScene();
}

void WXActivitScene::saveTempResData()
{
