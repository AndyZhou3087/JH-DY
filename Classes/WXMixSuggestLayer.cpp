...
WXMixSuggestLayer::WXMixSuggestLayer()
{
	startime0 = 0;
	startime1 = 1;
	startime2 = 0;
...
void WXMixSuggestLayer::initData()
{
	m_starnum = 0;
...
bool WXMixSuggestLayer::init()
{
	...
	freebtn->addTouchEventListener(CC_CALLBACK_2(WXMixSuggestLayer::onFree, this));
	freebtn->setEnabled(false);

	silverbtn = (cocos2d::ui::Button*)csbroot->getChildByName("silverbtn");
	silverbtn->addTouchEventListener(CC_CALLBACK_2(WXMixSuggestLayer::onSilver, this));

	goldbtn = (cocos2d::ui::Button*)csbroot->getChildByName("goldbtn");
	goldbtn->addTouchEventListener(CC_CALLBACK_2(WXMixSuggestLayer::onGold, this));

	cocos2d::ui::Button* backbtn = (cocos2d::ui::Button*)csbroot->getChildByName("backbtn");
	backbtn->addTouchEventListener(CC_CALLBACK_2(WXMixSuggestLayer::onBack, this));
...
void WXMixSuggestLayer::setStar(int num, bool isboss)
{
	m_starnum = num;
	m_isboss = isboss;
	std::string stagestr;
...
void WXMixSuggestLayer::getServerTime()
{
...
void WXMixSuggestLayer::setStageNum(int stage)
{
...
void WXMixSuggestLayer::onEnterTransitionDidFinish()
{
	Layer::onEnterTransitionDidFinish();
}

void WXMixSuggestLayer::onExit()
{
	Layer::onExit();
}

void WXMixSuggestLayer::hilight()
{
...
void WXMixSuggestLayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void WXMixSuggestLayer::disable()
{
	m_stageIcon->stopAllActions();
	m_node->setScale(1);
...
void WXMixSuggestLayer::onFree(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	CommonFuncs::BtnAction(pSender, type);
	if (type == ui::Widget::TouchEventType::ENDED)
	{
...
void WXMixSuggestLayer::onSilver(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	CommonFuncs::BtnAction(pSender, type);
	if (type == ui::Widget::TouchEventType::ENDED)
	{
...
void WXMixSuggestLayer::showLock(int starnum)
{
	if (lockNode == NULL)
	{
...
void WXMixSuggestLayer::onGold(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	CommonFuncs::BtnAction(pSender, type);
	if (type == ui::Widget::TouchEventType::ENDED)
	{
...
void WXMixSuggestLayer::removeLock()
{
	if (lockNode != NULL)
	{
...
void WXMixSuggestLayer::onSuccess()
{
	Director::getInstance()->getRunningScene()->removeChildByName("waitbox");

	severtime = GlobalData::servertime;

	this->unschedule(schedule_selector(WXMixSuggestLayer::updateServerTime));
	this->schedule(schedule_selector(WXMixSuggestLayer::updateServerTime), 1.0f);
...
void WXMixSuggestLayer::onErr(int errcode)
{
	Director::getInstance()->getRunningScene()->removeChildByName("waitbox");

}

void WXMixSuggestLayer::updateServerTime(float dt)
{
	severtime++;
...
void WXMixSuggestLayer::jumpDown(cocos2d::Node *node, float dt) {
	if (nullptr == node) {
		return;
	}
...
void WXMixSuggestLayer::randMixGf(int type)
{
...
void WXMixSuggestLayer::shake(Node * node, float scaleLarge, float scaleSmall) {
	if (NULL == node) {
		return;
	}
...
void WXMixSuggestLayer::loadMixGfUi(MixGfData mixdata)
{
	std::string suggestgfstr = GameDataSave::getInstance()->getSuggestMixGf();
...
void WXMixSuggestLayer::updateDesc()
{
...
void WXMixSuggestLayer::shake(Node * node) {
	if (NULL == node) {
		return;
	}
