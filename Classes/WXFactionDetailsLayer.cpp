...
\n捐赠1银两可获得1点贡献度。\
\n捐赠1元宝可获得10点贡献度。";
WXFactionDetailsLayer::WXFactionDetailsLayer()
{
}
...
void WXFactionDetailsLayer::petJump(cocos2d::Node *node, float dt, bool repeat, float intrval, int tag, ActionInterval *ac) {
	if (nullptr == node) {
		return;
	}
...
WXFactionDetailsLayer* WXFactionDetailsLayer::create()
{
	WXFactionDetailsLayer *pRet = new WXFactionDetailsLayer();
	if (pRet && pRet->init())
	{
		pRet->autorelease();
	}
...
bool WXFactionDetailsLayer::isPhone() {
...
bool WXFactionDetailsLayer::init()
{
	//蒙版
	...
	closebtn->addTouchEventListener(CC_CALLBACK_2(WXFactionDetailsLayer::onClose, this));
...
void WXFactionDetailsLayer::jelly(Node *node, bool repeat, float intrval, bool delay, int tag) {
	if (nullptr == node) {
		return;
	}
...
void WXFactionDetailsLayer::onClose(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void WXFactionDetailsLayer::jumpDown(cocos2d::Node *node, float dt) {
	if (nullptr == node) {
		return;
	}
