...
FactionPromotionLayer::FactionPromotionLayer()
{
	m_select = -1;
}
...
void FactionPromotionLayer::jump(cocos2d::Node *node, float dt, bool repeat, float intrval) {
	if (nullptr == node) {
		return;
	}
...
FactionPromotionLayer* FactionPromotionLayer::create(FactionMemberData *data, Node* target)
{
	FactionPromotionLayer *pRet = new FactionPromotionLayer();
...
void FactionPromotionLayer::jellyJump(cocos2d::Node *node, float dt, bool repeat, float intrval, int tag) {
	if (nullptr == node) {
		return;
	}
...
bool FactionPromotionLayer::init(FactionMemberData *data, Node* target)
{
	//蒙版
	...
	okbtn->addTouchEventListener(CC_CALLBACK_2(FactionPromotionLayer::onOk, this));

	cocos2d::ui::Button* backbtn = (cocos2d::ui::Button*)csbnode->getChildByName("backbtn");
	backbtn->addTouchEventListener(CC_CALLBACK_2(FactionPromotionLayer::onBack, this));
	...
	for (int i = 0; i < 3; i++)
	{
		...
		m_checkBox[i]->addEventListener(CC_CALLBACK_2(FactionPromotionLayer::checkBoxCallback, this));
...
void FactionPromotionLayer::petJump(cocos2d::Node *node, float dt, bool repeat, float intrval, int tag, ActionInterval *ac) {
	if (nullptr == node) {
		return;
	}
...
void FactionPromotionLayer::checkBoxCallback(cocos2d::Ref* pSender, cocos2d::ui::CheckBox::EventType type)
{
...
void FactionPromotionLayer::jelly(Node *node, bool repeat, float intrval, bool delay, int tag) {
	if (nullptr == node) {
		return;
	}
...
void FactionPromotionLayer::onOk(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void FactionPromotionLayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	CommonFuncs::BtnAction(pSender, type);
	if (type == ui::Widget::TouchEventType::ENDED)
	{
...
void FactionPromotionLayer::onSuccess()
{
...
void FactionPromotionLayer::jumpDown(cocos2d::Node *node, float dt) {
	if (nullptr == node) {
		return;
	}
...
void FactionPromotionLayer::onErr(int errcode)
{
    Director::getInstance()->getRunningScene()->removeChildByName("waitbox");
	HintBox* hintbox;
	if (errcode == -4)
