...
bool WXBookShelfLayer::init()
{
	...
	backbtn->addTouchEventListener(CC_CALLBACK_2(WXBookShelfLayer::onBack, this));
	...
	for (int i = 0; i < categoryBtnNode->getChildrenCount(); i++)
	{
		...
		btn->setTag(i);
		btn->addTouchEventListener(CC_CALLBACK_2(WXBookShelfLayer::onCategory, this));
...
void WXBookShelfLayer::onEnterTransitionDidFinish()
{
	Layer::onEnterTransitionDidFinish();
}

void WXBookShelfLayer::updateContent(int category)
{
		...
		for (unsigned int m = 0; m < map_bookData[vec_type[i]].size(); m++)
		{
			...
			if (boxstr.length() > 0)
			{
				...
				MenuItemSprite* boxItem = MenuItemSprite::create(
					...
					box,
					box,
					CC_CALLBACK_1(WXBookShelfLayer::onclick, this));
				boxItem->setTag(0);
...
void WXBookShelfLayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void WXBookShelfLayer::onclick(Ref* pSender)
{
	SoundManager::getInstance()->playSound(SoundManager::SOUND_ID_BUTTON);
	Node* node = (Node*)pSender;
	BookData* bdata = (BookData*)node->getUserData();
	this->addChild(WXBookDetailsLayer::create(bdata));
}

int WXBookShelfLayer::getCountByType(int type)
{
	int count = 0;
	return map_bookData[type].size();
}

void WXBookShelfLayer::loadBookData()
{
...
bool WXBookShelfLayer::larger_callback(BookData a, BookData b)
{
...
void WXBookShelfLayer::onCategory(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	CommonFuncs::BtnAction(pSender, type);
	if (type == ui::Widget::TouchEventType::ENDED)
	{
