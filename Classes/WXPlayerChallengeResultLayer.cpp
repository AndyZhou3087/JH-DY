...
WXPlayerChallengeResultLayer::WXPlayerChallengeResultLayer()
{
...
void WXPlayerChallengeResultLayer::initData()
{
	m_starnum = 0;
...
WXPlayerChallengeResultLayer* WXPlayerChallengeResultLayer::create(RankData* fightPlayerData, int win)
{
	WXPlayerChallengeResultLayer *pRet = new WXPlayerChallengeResultLayer();
...
void WXPlayerChallengeResultLayer::setStar(int num, bool isboss)
{
	m_starnum = num;
	m_isboss = isboss;
	std::string stagestr;
...
bool WXPlayerChallengeResultLayer::init(RankData* fightPlayerData, int win)
{
	...
	backbtn->addTouchEventListener(CC_CALLBACK_2(WXPlayerChallengeResultLayer::onBack, this));
	...
	if (win == 1)
	{
		...
		if (fightPlayerData->rank < GlobalData::myrank)
		{
			...
			this->scheduleOnce(schedule_selector(WXPlayerChallengeResultLayer::delayShowRank), 1.0f);
		}
...
void WXPlayerChallengeResultLayer::setStageNum(int stage)
{
...
void WXPlayerChallengeResultLayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	CommonFuncs::BtnAction(pSender, type);
	if (type == ui::Widget::TouchEventType::ENDED)
	{
		this->removeFromParentAndCleanup(true);
		WXRankLayer* rlayer = (WXRankLayer*)g_gameLayer->getChildByName("ranklayer");
		if (rlayer != NULL)
			rlayer->getRankData(2);
	}
}

void WXPlayerChallengeResultLayer::onSuccess()
{
	
}

void WXPlayerChallengeResultLayer::hilight()
{
...
void WXPlayerChallengeResultLayer::onErr(int errcode)
{
	HintBox * box = HintBox::create(CommonFuncs::gbk2utf("数据获取异常，请检查网络连接！！"));
	this->addChild(box);
}

void WXPlayerChallengeResultLayer::disable()
{
	m_stageIcon->stopAllActions();
	m_node->setScale(1);
...
void WXPlayerChallengeResultLayer::nomal()
{
	m_stageIcon->stopAllActions();
	m_node->setScale(1);
	std::string stagestr;
...
void WXPlayerChallengeResultLayer::showRank(float dt)
{
	...
	if (fabs(_myrank - _visualmyrank) < 1.0f) {
		...
		this->unschedule(schedule_selector(WXPlayerChallengeResultLayer::showRank));
	}
...
void WXPlayerChallengeResultLayer::showLock(int starnum)
{
	if (lockNode == NULL)
	{
...
void WXPlayerChallengeResultLayer::delayShowRank(float dt)
{
	this->schedule(schedule_selector(WXPlayerChallengeResultLayer::showRank), 1.0f / 30);

}

void WXPlayerChallengeResultLayer::removeLock()
{
	if (lockNode != NULL)
	{
