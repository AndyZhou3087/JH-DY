...
WXHSLJRankLayer::WXHSLJRankLayer()
{
...
WXHSLJRankLayer* WXHSLJRankLayer::create()
{
	WXHSLJRankLayer *pRet = new WXHSLJRankLayer();
	if (pRet && pRet->init())
	{
		pRet->autorelease();
	}
...
void WXHSLJRankLayer::setStar(int num, bool isboss)
{
	m_starnum = num;
	m_isboss = isboss;
	std::string stagestr;
...
bool WXHSLJRankLayer::init()
{
	LayerColor* color = LayerColor::create(Color4B(11, 32, 22, 160));
	this->addChild(color);

	m_csbnode = CSLoader::createNode("WXHSLJRankLayer.csb");
	this->addChild(m_csbnode);

	cocos2d::ui::Widget *backbtn = (cocos2d::ui::Widget*)m_csbnode->getChildByName("backbtn");
	backbtn->addTouchEventListener(CC_CALLBACK_2(WXHSLJRankLayer::onBack, this));
...
void WXHSLJRankLayer::setStageNum(int stage)
{
...
void WXHSLJRankLayer::onEnterTransitionDidFinish()
{
	Layer::onEnterTransitionDidFinish();
}

void WXHSLJRankLayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void WXHSLJRankLayer::getRankData()
{
...
void WXHSLJRankLayer::delayShowData(float dt)
{
...
void WXHSLJRankLayer::onSuccess()
{
	this->scheduleOnce(schedule_selector(WXHSLJRankLayer::delayShowData), 0.1f);

}

void WXHSLJRankLayer::showLock(int starnum)
{
	if (lockNode == NULL)
	{
...
void WXHSLJRankLayer::onErr(int errcode)
{
	Director::getInstance()->getRunningScene()->removeChildByName("waitbox");
...
void WXHSLJRankLayer::removeLock()
{
	if (lockNode != NULL)
	{
...
bool HSLJRankItem::init(HSLJRankData *data)
{
	...
	{
		WXHSLJRankLayer* rlayer = (WXHSLJRankLayer*)g_gameLayer->getChildByName("shljranklayer");
	...
	str = WXHSLJMainLayer::getDwStr(exp);
