...
WXRelationLayer::WXRelationLayer()
{
...
WXRelationLayer* WXRelationLayer::create()
{
	WXRelationLayer *pRet = new WXRelationLayer();
	if (pRet && pRet->init())
	{
		pRet->autorelease();
	}
...
bool WXRelationLayer::init()
{
	Node* csbnode = CSLoader::createNode("relationLayer.csb");
	this->addChild(csbnode);

	cocos2d::ui::Widget *backbtn = (cocos2d::ui::Widget*)csbnode->getChildByName("backbtn");
	backbtn->addTouchEventListener(CC_CALLBACK_2(WXRelationLayer::onBack, this));
...
void WXRelationLayer::onEnterTransitionDidFinish()
{
	Layer::onEnterTransitionDidFinish();
	this->scheduleOnce(schedule_selector(WXRelationLayer::delayShowData), 0.1f);
}

void WXRelationLayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void WXRelationLayer::delayShowData(float dt)
{
