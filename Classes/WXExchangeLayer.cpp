...
WXExchangeLayer::WXExchangeLayer()
{
...
void WXExchangeLayer::shake(Node * node, float scaleLarge, float scaleSmall) {
	if (NULL == node) {
		return;
	}
...
WXExchangeLayer* WXExchangeLayer::create(std::string npcid)
{
	WXExchangeLayer *pRet = new WXExchangeLayer();
...
bool WXExchangeLayer::init(std::string npcid)
{
	...
	backbtn->addTouchEventListener(CC_CALLBACK_2(WXExchangeLayer::onBack, this));

	m_exgbtn = (cocos2d::ui::Widget*)csbnode->getChildByName("exgbtn");
	m_exgbtn->addTouchEventListener(CC_CALLBACK_2(WXExchangeLayer::onExg, this));
	...
	this->scheduleOnce(schedule_selector(WXExchangeLayer::delayShowExgData), 0.2f);
...
void WXExchangeLayer::shake(Node * node) {
	if (NULL == node) {
		return;
	}
...
void WXExchangeLayer::onEnterTransitionDidFinish()
{
	Layer::onEnterTransitionDidFinish();
}

void WXExchangeLayer::delayShowExgData(float dt)
{
...
void WXExchangeLayer::onNpcGoodsItem(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	isClickMyGoods = false;
	...
	if (type == ui::Widget::TouchEventType::BEGAN)
	{
		...
		schedule(schedule_selector(WXExchangeLayer::longTouchUpdate), 1.0f);
		clickwhere = 4;
	}
	...
	else if (type == ui::Widget::TouchEventType::ENDED)
	{
		...
		unschedule(schedule_selector(WXExchangeLayer::longTouchUpdate));
		if (!m_isLongPress)
		{
...
void WXExchangeLayer::jump(cocos2d::Node *node, float dt, bool repeat, float intrval) {
	if (nullptr == node) {
		return;
	}
...
void WXExchangeLayer::updateMyGoods(PackageData data, int type)
{
	if (type == 0)
	{
		unsigned int i = 0;
...
void WXExchangeLayer::jellyJump(cocos2d::Node *node, float dt, bool repeat, float intrval, int tag) {
	if (nullptr == node) {
		return;
	}
...
void WXExchangeLayer::updateNpcGoods(PackageData data, int type)
{
	if (type == 1)
	{
		unsigned int i = 0;
...
void WXExchangeLayer::onMyGoodsItem(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void WXExchangeLayer::petJump(cocos2d::Node *node, float dt, bool repeat, float intrval, int tag, ActionInterval *ac) {
	if (nullptr == node) {
		return;
	}
...
void WXExchangeLayer::giveNpc(std::string strid)
{
...
void WXExchangeLayer::jelly(Node *node, bool repeat, float intrval, bool delay, int tag) {
	if (nullptr == node) {
		return;
	}
...
void WXExchangeLayer::giveHero(std::string strid)
{
...
int WXExchangeLayer::getCountByResId(std::string strid, int inwhere)
{
	int count = 0;
...
void WXExchangeLayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void WXExchangeLayer::onExg(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	CommonFuncs::BtnAction(pSender, type);
	if (type == ui::Widget::TouchEventType::ENDED)
	{
...
void WXExchangeLayer::jumpDown(cocos2d::Node *node, float dt) {
	if (nullptr == node) {
		return;
	}
...
void WXExchangeLayer::checkValue()
{
	if (isWxbExg)
	{
...
void WXExchangeLayer::updataMyGoodsUI()
{
	int size0 = myGoodsData.size();
	int size1 = npcExgData.size();
	int size = size0 + size1;
	...
	for (int i = 0; i < allsize; i++)
	{
		...
		boxItem->addTouchEventListener(CC_CALLBACK_2(WXExchangeLayer::onMyGoodsItem, this));
...
void WXExchangeLayer::updataNpcGoodsUI()
{
	int size0 = npcGoodsData.size();
	...
	for (int i = 0; i < allsize; i++)
	{
		...
		std::string name = StringUtils::format("pitem%d", i);
		cocos2d::ui::ImageView* boxItem = cocos2d::ui::ImageView::create(boxstr, cocos2d::ui::Widget::TextureResType::PLIST);
		boxItem->addTouchEventListener(CC_CALLBACK_2(WXExchangeLayer::onNpcGoodsItem, this));
		boxItem->setTouchEnabled(true);
		boxItem->setTag(i);
...
void WXExchangeLayer::onExit()
{
	Layer::onExit();
}

void WXExchangeLayer::randExchgRes(std::vector<std::string> &vec_exchgres)
{
...
void WXExchangeLayer::longTouchUpdate(float delay){
	m_isLongPress = true;
	if (m_longTouchNode != NULL){
