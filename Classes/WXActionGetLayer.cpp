...
WXActionGetLayer::WXActionGetLayer()
{
	repeatCount = 0;
}
...
bool WXActionGetLayer::init(int rid, std::vector<int> res_ids, int type, int actype)
{
	...
	m_backbtn->addTouchEventListener(CC_CALLBACK_2(WXActionGetLayer::onBack, this));

	//1："继续采集", 2："继续砍伐", 3："继续挖掘"按钮
	m_getbtn = (cocos2d::ui::Button*)csbnode->getChildByName("getbtn");
	m_getbtn->addTouchEventListener(CC_CALLBACK_2(WXActionGetLayer::onGet, this));
	...
	m_getallbtn->addTouchEventListener(CC_CALLBACK_2(WXActionGetLayer::onAllGet, this));
	...
	//点击后山列表中的操作获取一次资源
	this->scheduleOnce(schedule_selector(WXActionGetLayer::delayDoAction), 0.2f);
	////layer 点击事件，屏蔽下层事件
...
void WXActionGetLayer::playLeftLinkEffect(int src, int dst, int row, bool bRed) {
	if (src <= dst || row <0) {
		return;
	}
...
void WXActionGetLayer::doAction(float dt)
{
...
void WXActionGetLayer::playRightLinkEffect(int src, int dst, int row, bool bRed) {
	if (src >= dst || row <0) {
		return;
	}
...
void WXActionGetLayer::onRewardItem(cocos2d::Ref* pSender)
{
...
void WXActionGetLayer::playTopLinkEffect(int src, int dst, int col, bool bRed) {
	if (src <= dst || col <0) {
		return;
	}
...
void WXActionGetLayer::onPackageItem(cocos2d::Ref* pSender)
{
	SoundManager::getInstance()->playSound(SoundManager::SOUND_ID_BUTTON);
	//点击背包栏
	Node* node = (Node*)pSender;
...
void WXActionGetLayer::playBottomLinkEffect(int src, int dst, int col, bool bRed) {
	if (src >= dst || col <0) {
		return;
	}
...
WXActionGetLayer* WXActionGetLayer::create(int rid, std::vector<int> res_ids, int type, int actype)
{
	WXActionGetLayer *pRet = new WXActionGetLayer();
...
void WXActionGetLayer::clearBall(cocos2d::Vector<cocos2d::Sprite *> * vBall) {
	if (nullptr == vBall) {
		return;
	}
...
void WXActionGetLayer::onBack(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
...
void WXActionGetLayer::playShineEffect(cocos2d::Vector<cocos2d::Sprite *> vBall) {
...
void WXActionGetLayer::onGet(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	CommonFuncs::BtnAction(pSender, type);
...
void WXActionGetLayer::playTouchEffect(int row, int col) {
	if (row < 0 || row >= 6) {
		return;
	}
...
void WXActionGetLayer::delayDoAction(float dt)
{
	if (m_actype == 1)
	...
	{
		this->scheduleOnce(schedule_selector(WXActionGetLayer::doAction), 1.0f);
	}
}

void WXActionGetLayer::onAllGet(cocos2d::Ref *pSender, cocos2d::ui::Widget::TouchEventType type)
{
	CommonFuncs::BtnAction(pSender, type);
	//全部拾取
	if (type == ui::Widget::TouchEventType::ENDED)
...
void WXActionGetLayer::stopTouchEffect() {
...
void WXActionGetLayer::loadTempData()
{
	tempResData.clear();
	//临时存放的数据，保存在本地文件中，解析
...
void WXActionGetLayer::markSprite(int row, int col, bool bYes) {
	if (row < 0 || row >= 7) {
		return;
	}
...
void WXActionGetLayer::saveTempData()
{
...
void WXActionGetLayer::updataMyPackageUI()
{
	...
	for (int i = 0; i < MyPackage::getSize(); i++)
	{
		...
		MenuItemSprite* boxItem = MenuItemSprite::create(
			...
			box,
			box,
			CC_CALLBACK_1(WXActionGetLayer::onPackageItem, this));
		boxItem->setTag(i);
...
void WXActionGetLayer::clearMarkSprite() {
...
void WXActionGetLayer::clearAllBall() {
	clearBall(&m_vLeftBalls);
	clearBall(&m_vRightBalls);
	clearBall(&m_vTopBalls);
	clearBall(&m_vBottomBalls);
...
void WXActionGetLayer::updataRewardUI()
{
	for (unsigned int i = 0; i < getResData.size(); i++)
	{
		std::string boxstr = "ui/buildsmall.png";
		...
		MenuItemSprite* boxItem = MenuItemSprite::create(
			...
			box,
			box,
			CC_CALLBACK_1(WXActionGetLayer::onRewardItem, this));
		boxItem->setTag(i);
...
//退出也要保存下临时存放的数据
void WXActionGetLayer::onExit()
{
	saveTempData();
	Layer::onExit();
}

void WXActionGetLayer::setBtnStatus(bool enable)
{
...
void WXActionGetLayer::addEventText()
{
	int resrid = atoi(GlobalData::vec_resData[mrid].strid.c_str());
	std::string desc;
	...
	else
	{
		...
		if (resrid != 69)
			this->scheduleOnce(schedule_selector(WXActionGetLayer::showDotDot), 0.2f);
		g_uiScroll->addEventText(desc);
	}
	if (resrid == 69)//打水
	{
...
void WXActionGetLayer::showDotDot(float dt)
{
	g_uiScroll->addEventText("..........");
	this->scheduleOnce(schedule_selector(WXActionGetLayer::addEventText2), 1.0f);
}

void WXActionGetLayer::addEventText2(float dt)
{
	std::string desc;
...
void WXActionGetLayer::showNewerGuide(int step)
{
