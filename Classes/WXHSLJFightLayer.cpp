...
WXHSLJFightLayer::WXHSLJFightLayer()
{
	isUseWg = false;
	win = 0;
	nextmyhero = 1;
...
void WXHSLJFightLayer::initRandSeed() {
...
WXHSLJFightLayer* WXHSLJFightLayer::create(std::string addrid)
{
	WXHSLJFightLayer *pRet = new WXHSLJFightLayer();
...
time_t WXHSLJFightLayer::getNowTime()
{
...
bool WXHSLJFightLayer::init(std::string addrid)
{
...
long long WXHSLJFightLayer::getNowTimeMs() {
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID || CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
	struct timeval nowTimeval;
	gettimeofday(&nowTimeval, NULL);
...
void WXHSLJFightLayer::updateMyInfo()
{
	...
	this->scheduleOnce(schedule_selector(WXHSLJFightLayer::delayMyFight), 1.5f);//1.0s，hero->player
}

void WXHSLJFightLayer::updatePlayerInfo()
{
	...
	resetSkills();
	this->scheduleOnce(schedule_selector(WXHSLJFightLayer::delayPlayerFight), 1.5f);
}


bool WXHSLJFightLayer::isBeforeToday(time_t sec) {
	struct tm *tm;
#if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32)  
	//win32平台
	time_t timep;
...
std::string WXHSLJFightLayer::loadMyData(int herotype)
{
...
long long WXHSLJFightLayer::getTodayLeftSec() {
	long long nowSec = getNowTime();
	return (86400 - nowSec % 86400);
}

int WXHSLJFightLayer::getMyMaxLife(std::string localid)
{
...
bool WXHSLJFightLayer::getRandomBoolean(float rate) {
...
int WXHSLJFightLayer::getMyAtk(std::string localid)
{
	int weaponAtk = 0;
	int wgAtk = 0;
...
bool WXHSLJFightLayer::getRandomBoolean() {
...
int WXHSLJFightLayer::getMyDf(std::string localid)
{
	int adf = 0;
	int ngdf = 0;
...
int WXHSLJFightLayer::getRandomNum(int range) {
...
float WXHSLJFightLayer::getMyCrit(std::string localid)
{
	int hlv = GameDataSave::getInstance()->getHeroLV(localid);
...
int WXHSLJFightLayer::getRandomNum(int rangeStart, int rangeEnd) {
...
float WXHSLJFightLayer::getMyDodge(std::string localid)
{
	int hlv = GameDataSave::getInstance()->getHeroLV(localid);
...
void WXHSLJFightLayer::shake(Node * node, float scaleLarge, float scaleSmall) {
	if (NULL == node) {
		return;
	}
...
int WXHSLJFightLayer::getPlayerHurt()
{
...
void WXHSLJFightLayer::shake(Node * node) {
	if (NULL == node) {
		return;
	}
...
void WXHSLJFightLayer::mySkillComboAtk(float dt)
{
	...
	{
		playerlife = 0;
		this->unschedule(schedule_selector(WXHSLJFightLayer::mySkillComboAtk));
	}
	updatePlayerLife();
}

bool WXHSLJFightLayer::isPhone() {
...
void WXHSLJFightLayer::playerSkillComboAtk(float dt)
{
	...
	{
		mylife = 0;
		this->unschedule(schedule_selector(WXHSLJFightLayer::playerSkillComboAtk));
	}
	updateMyLife();
}

void WXHSLJFightLayer::delayMyFight(float dt)
{
	...
	else
	{
		if (checkMySkill(H_WG) == S_SKILL_3)
		{
			...
			if (count > 0)
			{
				...
				this->schedule(schedule_selector(WXHSLJFightLayer::mySkillComboAtk), 0.3f, count - 1, 0.2f);
			}
		}
		this->scheduleOnce(schedule_selector(WXHSLJFightLayer::delayPlayerFight), 1.5f);//延迟显示NPC 攻击，主要文字显示，需要看一下，所以延迟下
	}
}

void WXHSLJFightLayer::jump(cocos2d::Node *node, float dt, bool repeat, float intrval) {
	if (nullptr == node) {
		return;
	}
...
void WXHSLJFightLayer::delayPlayerFight(float dt)
{
	...
	if (isplayerAct != 1)
	{
		...
		{
			showMySkill(skilltype);
			this->scheduleOnce(schedule_selector(WXHSLJFightLayer::delayMyFight), 1.5f);
			return;
		}
	...
	if (mylife > 0.0f)
	{
		...
		if (checkPlayerSkill(H_WG) == S_SKILL_3)
		{
			...
			if (count > 0)
			{
				...
				this->schedule(schedule_selector(WXHSLJFightLayer::playerSkillComboAtk), 0.3f, count - 1, 0.2f);
			}
		}
	}
	else
...
void WXHSLJFightLayer::jellyJump(cocos2d::Node *node, float dt, bool repeat, float intrval, int tag) {
	if (nullptr == node) {
		return;
	}
...
int WXHSLJFightLayer::getMyHurt(std::string localid)
{
...
void WXHSLJFightLayer::resetSkills()
{
...
void WXHSLJFightLayer::delayShowResultLayer(float dt)
{
	WXHSLJFightResultLayer* layer = WXHSLJFightResultLayer::create(nextmyhero, win);
	g_gameLayer->addChild(layer, 5);
	this->removeFromParentAndCleanup(true);
}

void WXHSLJFightLayer::showFightWord(int type, int value)
{
	std::string wordstr;
	int size = 0;
	int r = 0;
...
void WXHSLJFightLayer::petJump(cocos2d::Node *node, float dt, bool repeat, float intrval, int tag, ActionInterval *ac) {
	if (nullptr == node) {
		return;
	}
...
void WXHSLJFightLayer::checkWordLblColor(std::string wordstr)
{
...
void WXHSLJFightLayer::jelly(Node *node, bool repeat, float intrval, bool delay, int tag) {
	if (nullptr == node) {
		return;
	}
...
std::string WXHSLJFightLayer::getMyGfFightStr(std::string wgstr)
{
	std::string retstr;
...
std::string WXHSLJFightLayer::getPlayerGfFightStr(std::string wgstr)
{
	std::string retstr;
...
void WXHSLJFightLayer::updateMyLife()
{
...
void WXHSLJFightLayer::updatePlayerLife()
{
	//NPC血量显示
	std::string hpstr = StringUtils::format("%d/%d", playerlife, playermaxhp);
	playerhpvaluetext->setString(hpstr);
...
void WXHSLJFightLayer::jumpDown(cocos2d::Node *node, float dt) {
	if (nullptr == node) {
		return;
	}
...
int WXHSLJFightLayer::checkMySkill(HeroAtrType gftype)
{
	int ret = S_SNONE;
...
int WXHSLJFightLayer::checkPlayerSkill(HeroAtrType gftype)
{
	int ret = S_SNONE;
	std::string gfstr = "";
	int lv = 0;
...
void WXHSLJFightLayer::showMySkill(int skilltype)
{
	if (skilltype >= S_SKILL_1 && skilltype <= S_SKILL_5)
	{
...
void WXHSLJFightLayer::initData()
{
	m_starnum = 0;
...
void WXHSLJFightLayer::showPlayerSkill(int skilltype)
{
	if (skilltype >= S_SKILL_1 && skilltype <= S_SKILL_5)
	{
		std::string str = StringUtils::format("ui/skill%dtext.png", skilltype);
...
void WXHSLJFightLayer::setStar(int num, bool isboss)
{
	m_starnum = num;
	m_isboss = isboss;
	std::string stagestr;
...
void WXHSLJFightLayer::showMyTextAmin(std::string filename)
{
...
void WXHSLJFightLayer::showPlayerTextAmin(std::string filename)
{
...
void WXHSLJFightLayer::setStageNum(int stage)
{
...
int WXHSLJFightLayer::getPlayerMaxLife(int hindex)
{
...
int WXHSLJFightLayer::getPlayerDf(int hindex)
{
	int adf = 0;
	int ngdf = 0;
	float slvdf = 0.0f;//强化等级加防
...
int WXHSLJFightLayer::getPlayerAtk(int hindex)
{
	int weaponAtk = 0;
	int wgAtk = 0;
	float slvAtk = 0.0f;//强化等级加攻
...
int WXHSLJFightLayer::getPlayerDodde(int hindex)
{
	int heroindex = hindex;
...
void WXHSLJFightLayer::showLock(int starnum)
{
	if (lockNode == NULL)
	{
...
int WXHSLJFightLayer::getPlayerCrit(int hindex)
{
	int heroindex = hindex;
...
void WXHSLJFightLayer::removeLock()
{
	if (lockNode != NULL)
	{
...
void WXHSLJFightLayer::showResultLayer(int result)
{
	win = result;
	if (result == 1)
	{
		...
		{
			this->scheduleOnce(schedule_selector(WXHSLJFightLayer::delayShowResultLayer), 1.0f);
		}
		else
		{
			nextplayerhero++;
